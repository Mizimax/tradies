...
SOURCE="$MT5_ROOT/MQL5/Experts/PropGuard/PropGuard.mq5"
SOURCE_DIR="$MT5_ROOT/MQL5/Experts/PropGuard"
SOURCE_FILE="PropGuard.mq5"
EX5="$MT5_ROOT/MQL5/Experts/PropGuard/PropGuard.ex5"
LOG="$MT5_ROOT/MQL5/Experts/PropGuard/PropGuard.log"
TMP_LOG="/tmp/propguard-metaeditor-compile.log"
...
require_file "$SOURCE" "PropGuard source"
require_dir "$INCLUDE_DIR" "MT5 include directory"
require_dir "$INCLUDE_DIR/PropGuard" "PropGuard include directory"
for include_file in \
  PropRules.mqh \
  RiskBudget.mqh \
  Engines.mqh \
  ...
  Executor.mqh \
  Telemetry.mqh; do
  require_file "$INCLUDE_DIR/PropGuard/$include_file" "PropGuard include file"
done
# PropGuard reuses GoldScalper's news-blackout parser (Filters.mqh includes
# <GoldScalper/NewsFilter.mqh>) -- confirm it is installed too.
require_file "$INCLUDE_DIR/GoldScalper/NewsFilter.mqh" "GoldScalper NewsFilter include (PropGuard dependency)"
...
echo "MetaEditor command-line compile did not update PropGuard.ex5." >&2
echo "Source:   $SOURCE" >&2
echo "Compiled: $EX5" >&2
echo "Include:  $INCLUDE_ROOT" >&2
running_mt5="$(mt5_processes)"
