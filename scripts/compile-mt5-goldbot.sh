...
SOURCE_DIR="$MT5_ROOT/MQL5/Experts/GoldBot"
SOURCE_FILE="GoldBot.mq5"
EX5="$MT5_ROOT/MQL5/Experts/GoldBot/GoldBot.ex5"
LOG="$MT5_ROOT/MQL5/Experts/GoldBot/GoldBot.log"
TMP_LOG="/tmp/goldbot-metaeditor-compile.log"
...
require_file "$SOURCE" "GoldBot source"
require_dir "$INCLUDE_DIR" "MT5 include directory"
require_dir "$INCLUDE_DIR/GoldBot" "GoldBot include directory"
for include_file in \
  EntryFilters.mqh \
  Indicators.mqh \
  Risk.mqh \
  SMC.mqh \
  TradeManager.mqh; do
  require_file "$INCLUDE_DIR/GoldBot/$include_file" "GoldBot include file"
done
...
for _ in $(seq 1 60); do
  ...
  if ! kill -0 "$metaeditor_pid" 2>/dev/null; then
    echo "MetaEditor exited without updating GoldBot.ex5." >&2
