...
SOURCE="$MT5_ROOT/MQL5/Experts/DZV_Style_ADR/DZV_Style_ADR_EA.mq5"
SOURCE_DIR="$MT5_ROOT/MQL5/Experts/DZV_Style_ADR"
SOURCE_FILE="DZV_Style_ADR_EA.mq5"
EX5="$MT5_ROOT/MQL5/Experts/DZV_Style_ADR/DZV_Style_ADR_EA.ex5"
LOG="$MT5_ROOT/MQL5/Experts/DZV_Style_ADR/DZV_Style_ADR_EA.log"
TMP_LOG="/tmp/dzv-style-adr-metaeditor-compile.log"
INCLUDE_DIR="$MT5_ROOT/MQL5/Include"
...
print_compile_logs() {
  ...
  [[ -f "$TMP_LOG" ]] && print_text_file "$TMP_LOG" || echo "  missing"
  echo
  echo "MetaEditor compile log: $LOG"
  [[ -f "$LOG" ]] && print_text_file "$LOG" || echo "  missing"
}
...
require_file "$SOURCE" "DZV EA source"
for include_file in \
  DZVTypes.mqh \
  ADRCalculator.mqh \
  DailyState.mqh \
  ...
  AlertManager.mqh; do
  require_file "$INCLUDE_DIR/DZVStyle/$include_file" "DZV include file"
done
...
echo "Compiling DZV_Style_ADR..."
for _ in $(seq 1 60); do
  sleep 1
  after="$(stat -f '%m' "$EX5" 2>/dev/null || echo 0)"
  if [[ "$after" != "0" && "$after" != "$before" && -f "$EX5" ]]; then
    echo "DZV_Style_ADR compiled: $EX5"
    [[ -f "$LOG" ]] && print_text_file "$LOG"
    exit 0
  fi
  if ! kill -0 "$metaeditor_pid" 2>/dev/null; then
    echo "MetaEditor exited without updating DZV_Style_ADR_EA.ex5." >&2
...
echo "MetaEditor command-line compile did not update DZV_Style_ADR_EA.ex5." >&2
print_compile_logs >&2
exit 1
