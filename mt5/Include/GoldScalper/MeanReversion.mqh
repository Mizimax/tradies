...
int GoldScalperMeanReversionSignal(const string symbol,
                                    ...
                                    const ENUM_REGIME regimeMode = REGIME_RANGE)
{
   ...
   //--- 5b. Adaptive RSI Thresholds based on Regime
   double currentOverbought = (double)rsiOverbought;
   double currentOversold   = (double)rsiOversold;
   
   if(regimeMode == REGIME_BULL)
   {
      currentOversold = 35.0; // looser, catch more dips
      currentOverbought = 80.0; // tighter, fewer bad sells
   }
   else if(regimeMode == REGIME_BEAR)
   {
      currentOversold = 20.0; // tighter, fewer bad buys
      currentOverbought = 65.0; // looser, catch more rallies
   }

   //--- 6. Evaluate BUY signal
   bool bbBuyCondition  = m5Close < bbLower;
   bool rsiBuyCondition = rsi < currentOversold;
   bool trendBuyFilter  = m15Close > emaValue;
   bool regimeBuyFilter = GoldScalperRegimeAllowsSignal(regimeMode, +1);

   //--- 7. Evaluate SELL signal
   bool bbSellCondition  = m5Close > bbUpper;
   bool rsiSellCondition = rsi > currentOverbought;
   bool trendSellFilter  = m15Close < emaValue;
   bool regimeSellFilter = GoldScalperRegimeAllowsSignal(regimeMode, -1);
   ...
   Print("[MR-Signal] M5 close=", DoubleToString(m5Close, (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS)),
         ...
         " RegimeMode=", EnumToString(regimeMode));

   if(bbBuyCondition && rsiBuyCondition && trendBuyFilter && regimeBuyFilter)
   {
      Print("[MR-Signal] BUY signal: M5 close below lower BB, RSI=",
            DoubleToString(rsi, 2), " < ", DoubleToString(currentOversold, 1),
            ", M15 trend bullish (close > EMA)");
      return +1;
   }

   if(bbSellCondition && rsiSellCondition && trendSellFilter && regimeSellFilter)
   {
      Print("[MR-Signal] SELL signal: M5 close above upper BB, RSI=",
            DoubleToString(rsi, 2), " > ", DoubleToString(currentOverbought, 1),
...
//+------------------------------------------------------------------+
void GoldScalperMRTrailingStop(const string symbol,
                               const long magic,
                               CTrade &trade,
                               const double trailBETriggerMult = 1.0)
{
   if(trailBETriggerMult <= 0.0)
      return;

   // Read ATR for trigger distance
   double atr = GoldScalperMRBufferValue(s_mrAtrHandle, 0, 1);
   if(atr == EMPTY_VALUE || atr <= 0.0)
      return;
      
   double triggerDist = atr * trailBETriggerMult;
   int digits = (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS);
   double point = SymbolInfoDouble(symbol, SYMBOL_POINT);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(modify)
      {
         ...
         {
            Print("[MR-Trail] SL moved to BE for ticket=", ticket, 
                                     ...
                                     const double trailBETriggerMult = 1.0)
{
   ...
   if(trailBE)
   {
      GoldScalperMRTrailingStop(symbol, magic, trade, trailBETriggerMult);
   }
}
